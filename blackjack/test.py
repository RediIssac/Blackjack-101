...
window_title= "Card Detector"
cv2.namedWindow(window_title, cv2.WINDOW_NORMAL)


# Load names classes
classes = None
file = open(card_classes, "r")
# temp = file.readlines()
classes = [line.strip() for line in file.readlines()]
print(classes)
# with open(card_weight, 'r') as f:
    # print(f)
    # print(f.readlines())
    # classes = f.readlines()
...
net = cv2.dnn.readNet(card_weight,card_cfg)
...
while cv2.waitKey(1) < 0 or False:
    ...
    cv2.putText(image, label, (0, 15), cv2.FONT_HERSHEY_PLAIN, 1, (255, 0, 0))
